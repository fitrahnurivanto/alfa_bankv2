...
<div class="container mx-auto px-4 py-6 max-w-3xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-green-50 to-blue-50 px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-bold text-gray-800">
                <i class="fas fa-plus-circle mr-2 text-green-600"></i>Tambah Pelatihan Baru
            </h2>
            <p class="text-sm text-gray-600 mt-1">Isi formulir di bawah untuk menambah pelatihan</p>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.trainings.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select name="type" 
                        id="type"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent @error('type') border-red-500 @enderror"
                        required>
                    <option value="">Pilih Tipe</option>
                    <option value="corporate" {{ old('type') == 'corporate' ? 'selected' : '' }}>Corporate Training</option>
                    <option value="reguler" {{ old('type') == 'reguler' ? 'selected' : '' }}>Regular Training</option>
                    <option value="private" {{ old('type') == 'private' ? 'selected' : '' }}>Private Training</option>
                </select>
                @error('type')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" 
                          ...
                          placeholder="Deskripsi singkat tentang pelatihan ini...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
    ...
    <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div class="flex items-start">
            ...
            <div>
                <h4 class="font-semibold text-blue-800 mb-1">Tentang Pelatihan</h4>
                <p class="text-sm text-blue-700">
                    Pelatihan adalah materi atau program yang akan diajarkan dalam kelas. 
                    Setiap pelatihan harus dikategorikan untuk memudahkan pengelolaan dan pencarian.
                </p>
            </div>
        </div>
    </div>
